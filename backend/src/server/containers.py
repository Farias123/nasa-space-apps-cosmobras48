...
def build_db_config(config: Configuration) -> None:
...
def build_server_config(config: Configuration) -> None:
    ...
    config.server_api_root_version.from_env("SERVER_API_ROOT_VERSION", as_=str, default="v1")
    config.server_sysadmin_contact.from_env("SERVER_SYSADMIN_CONTACT", as_=str)


def build_swagger_ui_config(config: Configuration) -> None:
    ...
    config.sui_redoc_logo_path.from_env("SWAGGER_UI_REDOC_LOGO_PATH", as_=str, required=True)
    config.sui_redoc_logo_alt_text.from_env("SWAGGER_UI_REDOC_LOGO_ALT_TEXT", as_=str, required=True)
    config.sui_redoc_logo_background_color.from_env("SWAGGER_UI_REDOC_LOGO_BACKGROUND_COLOR", as_=str, required=True)
    config.sui_redoc_logo_href.from_env("SWAGGER_UI_REDOC_LOGO_HREF", as_=str, required=True)
    config.sui_redoc_logo_width.from_env("SWAGGER_UI_REDOC_LOGO_WIDTH", as_=str, required=True)
    config.sui_redoc_logo_height.from_env("SWAGGER_UI_REDOC_LOGO_HEIGHT", as_=str, required=True)


class Container(DeclarativeContainer):
    config = Configuration()
    
    build_db_config(config)
    build_server_config(config)
    build_swagger_ui_config(config)
